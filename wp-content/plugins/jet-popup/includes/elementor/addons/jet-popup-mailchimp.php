...
class Jet_Popup_Mailchimp extends Jet_Popup_Base {
	...
	protected function render() {
		...
		if ( filter_var( $use_icon, FILTER_VALIDATE_BOOLEAN ) ) {
			$icon_html = sprintf( '<i class="jet-popup-mailchimp__submit-icon %s"></i>', esc_attr( $button_icon ) );
		}
		...
		$email_id = 'jet-popup-mailchimp-mail-'. esc_attr( $widget_id );
		...
		$this->add_render_attribute( 'email-field-container',
			[
				...
				'data-column'        => esc_attr( $this->get_settings( 'email_field_size' ) ),
				'data-column-tablet' => esc_attr( $this->get_settings( 'email_field_size_tablet' ) ),
				'data-column-mobile' => esc_attr( $this->get_settings( 'email_field_size_mobile' ) ),
			]
		);
		...
		$this->add_render_attribute( 'email-input',
			[
				...
				'type'               => 'email',
				'name'               => 'email',
				'placeholder'        => esc_attr( $this->get_settings( 'email_placeholder' ) ),
					...
					<div <?php echo $this->get_render_attribute_string( 'email-field-container' ); ?>><?php
						if ( ! empty( $this->get_settings( 'email_label' ) ) ) {
							?><label <?php echo $this->get_render_attribute_string( 'email-label' ); ?>><?php
								echo wp_kses_post( $this->get_settings( 'email_label' ) );
							?></label><?php
						}
						?><input <?php echo $this->get_render_attribute_string( 'email-input' ); ?>>
					</div>

					<?php wp_kses_post( $this->generate_additional_fields() ); ?>
					<div class="jet-popup-mailchimp__field-container jet-popup-mailchimp__submit-container">
						<?php echo sprintf(
							'<a class="jet-popup-mailchimp__submit elementor-button elementor-size-md" href="#">%s<span class="jet-popup-mailchimp__submit-text">%s</span></a>',
							$icon_html,
